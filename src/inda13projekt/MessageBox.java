...
public class MessageBox {
	...
	public void render(Graphics g) {
		...
		ttf20.drawString(x + 10 - width / 2, y + 10 - height / 2, name + ":",
				Color.black);

		for (int j = 0; j < lines; j++) {
			if (j == lines - 1)
				ttf20.drawString(x + 10 - width / 2, y + 33 + (23 * j) - height
						/ 2, message.substring(j * lineLength), Color.black);
			else
				ttf20.drawString(
						x + 10 - width / 2,
						y + 33 + (23 * j) - height / 2,
						message.substring(j * lineLength, (j + 1) * lineLength),
						Color.black);
		}
	}
